...
router.post('/', requirePermission('vendors.manage'), validate(bodySchema), async (req, res, next) => {
  try {
...
router.patch('/:id', requirePermission('vendors.manage'), validate(bodySchema.partial()), async (req, res, next) => {
  try {
...
mountCrudExtensions(router, {
  Model: Vendor,
  manageMiddleware: requirePermission('vendors.manage'),

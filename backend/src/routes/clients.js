...
router.post('/', requirePermission('clients.manage'), validate(bodySchema), async (req, res, next) => {
  try {
...
router.patch('/:id', requirePermission('clients.manage'), validate(bodySchema.partial()), async (req, res, next) => {
  try {
...
mountCrudExtensions(router, {
  Model: Client,
  manageMiddleware: requirePermission('clients.manage'),
